...
// This file was modified by Thierry Danard
...
function initRTE(imgPath, incPath, css, gotoArticlePath, gotoImagePath, contentStyle) {
	//set browser vars
	...
	cssFile = css;
	gotoArtPath = gotoArticlePath;
    gotoImgPath = gotoImagePath;
    cttStyle = contentStyle;
...
function writeDefault(rte, html, width, height, buttons, readOnly) {
	if (!readOnly) {
		document.writeln('<textarea class="' + cttStyle + '" name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;">' + html + '</textarea>');
	} else {
		document.writeln('<textarea class="' + cttStyle + '" name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;" readonly>' + html + '</textarea>');
	}
}
...
function writeRTE(rte, html, width, height, buttons, readOnly) {
	...
	if (buttons == true) {
		...
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'hyperlink.gif" width="25" height="24" alt="Insert Link for selected text" title="Insert Link for selected text" onClick="FormatText(\'' + rte + '\', \'createlink\')"></td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'image.gif" width="25" height="24" alt="Insert Image" title="Insert Image" onClick="AddImage(\'' + rte + '\')"></td>');
...
function enableDesignMode(rte, html, readOnly) {
	...
	if (document.all) {
		...
		oRTE.src=includesPath + "blank.htm"; 
		oRTE.open(); 
...
function FormatText(rte, command, option) {
	...
	try {
		if ((command == "forecolor") || (command == "hilitecolor")) {
			...
			var szURL = prompt("Enter a URL to turn your selected text into a link:", gotoArtPath);
			try {
...
function AddImage(rte) {
	...
	imagePath = prompt("Enter the URL of the image to insert:", gotoImgPath);			
