...
import com.corendal.netapps.wiki.readdataelements.RequestStatusTypeElements;

/**
 * AbstractRequestStatusTypeLoadHandler is the abstract class processing XML
 * entities representing a request status type found after a load query.
 * 
 * @version $Id: AbstractRequestStatusTypeLoadHandler.java,v 1.1 2005/09/06
 *          21:25:33 tdanard Exp $
 */
public abstract class AbstractRequestStatusTypeLoadHandler extends
        AbstractLoadHandler implements RequestStatusTypeLoadHandler {
    /** Name in the request status type XML entity. */
    private StringBuilder name;

    /** Description in the request status type XML entity. */
    private StringBuilder description;

    /** Comment in the request status type XML entity. */
    private StringBuilder comment;

    /** Order in the request status type XML entity. */
    private int order;

    /** Enabled-flag in the request status type XML entity. */
     ...
     */
    protected AbstractRequestStatusTypeLoadHandler() {
        // parent class constructor is called
    }

    /*
     * @see com.corendal.netapps.wiki.interfaces.RequestStatusTypeLoadHandler#getName()
     */
    ...
    /*
     * @see com.corendal.netapps.wiki.interfaces.RequestStatusTypeLoadHandler#getDescription()
     */
    ...
    /*
     * @see com.corendal.netapps.wiki.interfaces.RequestStatusTypeLoadHandler#getComment()
     */
    ...
    /*
     * @see com.corendal.netapps.wiki.interfaces.RequestStatusTypeLoadHandler#getOrder()
     */
    public int getOrder() {
        return this.order;
    }
    ...
    /*
     * @see com.corendal.netapps.wiki.interfaces.RequestStatusTypeLoadHandler#getEnabledFlag()
     */
    ...
    protected void processElement(String namespaceUri, String localName,
        ...
        super.processElement(namespaceUri, localName, qualifiedName,
                attributes, isStart);
        if (qualifiedName.equals(RequestStatusTypeElements.ENTITY)) {
            if (isStart) {
                this.enabledFlag = this.getStringAttributeValue(attributes,
                        RequestStatusTypeElements.ENABLED_FLAG);
                this.order = this.getIntegerAttributeValue(attributes,
                        RequestStatusTypeElements.ORDER);
            }
        } else if (qualifiedName.equals(RequestStatusTypeElements.NAME)) {
            this.isNameCollected = isStart;
        } else if (qualifiedName.equals(RequestStatusTypeElements.DESCRIPTION)) {
            this.isDescriptionCollected = isStart;
        } else if (qualifiedName.equals(RequestStatusTypeElements.COMMENT)) {
