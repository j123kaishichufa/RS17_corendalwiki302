...
/**
 * AbstractArticleAccessGroupEntryBlock is the parent block common to all
 * article viewing blocks.
 * 
 * @version $Id: AbstractArticleAccessGroupEntryBlock.java,v 1.1 2005/09/06
 *          21:25:28 tdanard Exp $
 */
public abstract class AbstractArticleAccessGroupEntryBlock extends
        AbstractStandardEntryFormBlock implements Cloneable {
    /** Fully qualified name of this class. */
    private static final String ABSTRACT_CLASS_NAME = "com.corendal.netapps.wiki.blocks.AbstractArticleAccessGroupEntryBlock";
     ...
     */
    protected AbstractArticleAccessGroupEntryBlock() {
        this.article = null;
    }
    ...
    @Override
    public Object clone() {
        return (AbstractArticleAccessGroupEntryBlock) super.clone();
    }
    ...
    private void build() {
        ...
        PrimaryKey parentArticlePk = this.article.getMainParentPrimaryKey();
        ...
        if (this.getAccessType().equals(AccessTypeConstants.NEW)) {
            ...
            List<StandardGroup> allowedGroups = sgf
                    .findEnabledByRecordAndRolePrimaryKeys(articlePk,
    ...
    protected void buildNewRecord() {
         ...
         */
        if ((this.article.getIsFound()) && (this.article.getIsVisible())) {
            /*
             * print the form
             */
            if (resm.getIsRequestRequirementWaived(sa.getPrimaryKey(),
                ...
                // 1
                // line
                PrimaryKey noArticleAccessGroupEditingPk = PrimaryKeyUtil
                        .getAlphanumericSingleKey(MessagesDictionary.ERR_NO_ARTICLE_ACCESS_GROUP_EDITING);
                StandardFormMessage noArticleAccessGroupEditingMessage = (StandardFormMessage) sfmf
                        .findByPrimaryKey(noArticleAccessGroupEditingPk);
                this.addStandardFormObject(noArticleAccessGroupEditingMessage,
                        1, 1);
            }
        } else {
    ...
    protected void buildRemoveRecord() {
        ...
        if ((this.article.getIsFound()) && (this.article.getIsVisible())) {
            ...
            if (resm.getIsRequestRequirementWaived(sa.getPrimaryKey(),
                    this.article.getPrimaryKey())) {
                /*
                 * verify that the article has been added to
                 */
                if (groups.size() > 0) {
                    this.build();
                } else {
                    ...
                    // 1
                    // line
                    PrimaryKey noArticleAccessGroupPk = PrimaryKeyUtil
                            .getAlphanumericSingleKey(MessagesDictionary.ERR_NO_ARTICLE_ACCESS_GROUP);
                    StandardFormMessage noArticleAccessGroupMessage = (StandardFormMessage) sfmf
                            .findByPrimaryKey(noArticleAccessGroupPk);
                    StandardMessage articleMessage = noArticleAccessGroupMessage
                            .getStandardMessage();
                    articleMessage.replaceMessageText(this.article
                            .getNameText(), 1);
                    articleMessage.replaceMessageEncoded(this.article
                            .getNameEncoded(), 1);
                    articleMessage.replaceMessageHTML(this.article
                            .getNameHTML(), 1);
                    this.addStandardFormObject(noArticleAccessGroupMessage, 1,
                            1);
                }
            } else {
                this.setMaxSizes(1, 1, CanvasTypeConstants.CENTER); // 1
    ...
    public void validateExtraNewRecord() throws IOException {
        ...
        if (this.getIsValid()) {
            StandardContent content = (StandardContent) srf
                    .findByPrimaryKey(this.article.getPrimaryKey());
            PrimaryKey groupPk = groups.get(0);
            ...
            if (content.getHasAccessGroup(groupPk)) {
                 ...
                 */
                PrimaryKey errorPk = PrimaryKeyUtil
                        .getAlphanumericSingleKey(MessagesDictionary.ERR_ALREADY_ARTICLE_ACCESS_GROUP);
                StandardMessage error = (StandardMessage) smf
                        .findByPrimaryKey(errorPk);
        ...
        if (this.getIsValid()) {
            ...
            if (this.denyAccess.equals("Y")) {
                this.article.addDeniedAccessGroup(groupPk);
            } else {
                this.article.addAllowedAccessGroup(groupPk);
            }

            this.setIsValid(this.article.getIsDone());

            if (!(this.article.getIsDone())) {
                MessageUtil.printBufferInternalErrorHTML(ABSTRACT_CLASS_NAME,
                        this.article.getStoreTrace());
            }
        }
    ...
    public void validateExtraRemoveRecord() throws IOException {
        ...
        if (this.getIsValid()) {
            ...
            if (!(content.getHasAccessGroup(groupPk))) {
                 ...
                 */
                PrimaryKey errorPk = PrimaryKeyUtil
                        .getAlphanumericSingleKey(MessagesDictionary.ERR_NOT_ARTICLE_ACCESS_GROUP);
                StandardMessage error = (StandardMessage) smf
                        .findByPrimaryKey(errorPk);
        ...
        if (this.getIsValid()) {
            PrimaryKey groupPk = groups.get(0);
            this.article.removeAccessGroup(groupPk);
            this.setIsValid(this.article.getIsDone());
     ...
     */
    public void setStandardArticle(StandardArticle sd) {
        this.article = sd;
    }
...
// end AbstractArticleAccessGroupEntryBlock
